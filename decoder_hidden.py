...
def decoder_hidden(
    ...
    f_decoderh = theano.function([x,x_mask,y,y_mask],decoderh,name='f_decoderh')
    ...
    for x, y in train:
            ...
            hh = f_decoderh(x,x_mask,y,y_mask)
            hh = hh * y_mask[:,:,None]
            hh_mean = hh.sum(axis=0)
            
            with open('features/hidden/train.en-es.decoderh','a+') as fp:
                for hh_data in hh_mean:
    ...
    for x, y in test:
            ...
            with open('features/hidden/test.en-es.decoderh','a+') as fp:
                for hh_data in hh_mean:
                    fp.writelines('\t'.join(map(lambda x:str(x), list(hh_data)))+'\n')
